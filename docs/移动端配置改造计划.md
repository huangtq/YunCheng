...
> **一期范围：只做标准九宫格（standard）**  
> 热区 JSON / image-map / tile / 欢迎大图 / BGM 全部后置到二期。  
> 本期目标：后台配置九宫格模板与菜单后，移动端详情页能正确按 1/2/3 列、纯图标/图文样式渲染，并消费倒计时样式、主题色、报名人数、底部栏图标。
...
| `side_menu_enabled` | char(1) | 内容页是否显示侧栏 |
| `side_menu_json` | text | 侧栏菜单项（可先 JSON，后迁表） |
